...
exports.getUser = async (req, res) => {
  ...
  try {
    if (Object.keys(req.query).length > 0) {
      ...
      return res.status(400).send({ message: "Query parameters are not allowed" });
    }
...
exports.createUser = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    statsDClient.increment('endpoints.response.http.post.fail.validationError.createUser');
  ...
  try {
    ...
    if (existingUser) {
      ...
      statsDClient.increment('endpoints.response.http.post.fail.userExists.createUser');
...
exports.updateUser = async (req, res) => {
  ...
  if (Object.keys(req.query).length > 0 || Object.keys(req.body).length === 0) {
    ...
    statsDClient.increment('endpoints.response.http.put.fail.requestValidation.updateUser');
  ...
  if (!errors.isEmpty()) {
    ...
    statsDClient.increment('endpoints.response.http.put.fail.validationError.updateUser');
